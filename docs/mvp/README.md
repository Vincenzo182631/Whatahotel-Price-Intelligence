...
**Source proposal:** *WhataHotel Price Intelligence Proposal v2* (Aug 2026), prepared for Greg Guiteras, Lorraine Travel.
...
2. **No number is arbitrary.** Every weight and threshold in this spec is a *starting prior* with a stated rationale, lives in a versioned config record, and is calibratable without a code change. See doc 10.
...
4. **Confidence gates action.** Low confidence must degrade the recommendation, not the score's appearance of precision. **WAIT is never emitted below the confidence floor** — this is enforced at the engine boundary *and* asserted as a property test.
...
You have confirmed the data exists. I could not inspect it — the repository contains no schema, API definition, sample payload, or data dictionary. Each item below is a dependency this specification rests on. **These must be confirmed before implementation begins**; several would materially change the design.
...
- Effective Stay Value as a *full* comparison surface (Phase 2) — MVP computes the factor but the side-by-side UI is later
...
| D2 | Should benefits adjust the *baseline* or only factor F6? | F6 only in v1 (doc 02 §F6) | Consistency of historical comparisons |
