...
`subject.room_options` lists every room type with a live rate for that exact stay, cheapest first, each with its own nightly price. Re-request with `room_type_id` to score any of them: the comp set, the nearby-date series and the terms match are all keyed on the chosen room, so a different category returns a genuinely different score rather than the same one relabelled. Measured on hotel 2008 (Sep 8–11): the entry room scores 7.9 STRONG against a Comp-Set Index of 70.7, the Garden Suite 1.1 PREMIUM against 159. `subject.room_type.selected_by` says whether the room was the engine's pick (`ENGINE`, the cheapest) or the caller's (`USER`).
...
`signals.comp_set.room_match` says how closely the competitors' ROOMS match the one being scored: `CLASS_AND_VIEW` is a genuine like-for-like, `CLASS` drops the view, and `ANY` means no equivalent category is sold in that market and the comparison is against whatever is. `ANY` is a disclosure, never an equivalence claim.
