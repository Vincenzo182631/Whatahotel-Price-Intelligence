...
Caveats: `LIMITED_HISTORY`, `BASELINE_WIDENED`, `HIGH_VOLATILITY`, `STALE_DATA`, `WEAK_ROOM_MATCH`, `SINGLE_SOURCE`, `NO_COMPARABLES`, `UNRESOLVED_RATE_TERMS`. (`SHORT_LEAD_TIME` was retired in config v4 — its text was a forecast.)
...
| V2  | **Verdict consistency**  | Output must not contain language contradicting the recommendation checked against a phrase list per verdict. Since config v4 this also rejects any predictive phrasing — "will rise", "before it goes up", "unlikely to soften" — under every verdict, because the engine makes no claim about future price. Invariant P11 asserts the same over generated inputs.                                                                                          |
| V3  | **No data apologies**    | Output must not narrate an internal system condition — "not enough data", "could not verify", "insufficient", "unable to score", "unfortunately" (owner directive, 2026-08-26). Customer prose describes the hotel and the rate; the system's own limitations are carried in structured fields (`intelligence_mode`, `unavailable_reason`), never in sentences. Honest caveats stated about the product ("the rates do not state what each includes") pass. |
