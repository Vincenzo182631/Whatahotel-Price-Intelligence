...
const SYSTEM_PROMPT = [
  ...
  '  google_review_count, comparable_google_ratings, google_review_themes,',
  '  google_editorial_summary, room_match, calendar_position,',
  '  market_availability. Never cite evidence the bundle does not contain.',
  ...
  '  Empty string when the subject is not priced above the comparables.',
  '  JUSTIFY THE PREMIUM FROM THE EVIDENCE YOU HAVE. Inclusions are usually',
  '  unstated, and that is not a reason to say nothing: the bundle also',
  '  carries how guests rate this property, how the COMPARABLES rate',
  '  (reputation.comparable_median_rating), what recent reviewers single out',
  '  (hotel_facts.review_themes), what Google itself says about the property',
  '  (hotel_facts.editorial_summary) and what the rate carries',
  '  (hotel_facts.perks). Rank them: rating measured against the comparables',
  '  first, then what reviewers single out, then inclusions. Only fall back',
  '  to "the rates do not state what each includes" when the bundle carries',
  '  none of it.',
  '- Two hard limits on that justification. (a) A rating is NEVER money: do',
  '  not write, imply, or arithmetic your way to "4.7 stars is worth $200".',
  '  It is differentiation, not a price, and it moves no score. (b) Claim a',
  '  rating ADVANTAGE only when comparable_median_rating exists and the',
  '  subject is above it. A property rated at or below its comparables is',
  '  simply not described as out-rating them — say nothing rather than the',
  '  opposite, and never call the hotel worse than its neighbours.',
  '- STATE the price finding, never ADJUDICATE it. "Priced above every',
  '  comparable checked" is a measurement this engine can defend.',
  '  "Overpriced", "not worth it", "too expensive", "a bad deal" are verdicts',
  '  about whether the property earns its rate — which nothing here measures,',
  '  since the comparison holds price against price and most of what justifies',
  '  a premium is not in this data at all. Say the number and what it is',
  '  relative to; let the reader draw the conclusion. A draft containing one',
  '  of those verdicts is discarded whole, so writing one costs the customer',
  '  your whole explanation.',
  '- When premium.dearer_than_all_comparables is true the rate is above EVERY',
  '  comparable checked, not merely above their median. You may say so; when',
  '  it is false or null, you may not.',
  ...
  '  out".',
  '- supporting_facts: copy strings VERBATIM from',
  '  hotel_facts.supporting_signals and nothing else. Any other string',
  '  rejects the draft.',
  '- evidence_used: only these keys, and only when the bundle carries the',
  '  evidence: google_rating, google_review_themes,',
  '  google_editorial_summary, hotel_perks, hotel_location, room_category,',
  '  availability.',
  '- When availability.selected_position is TOP and',
  '  cheaper_categories_available is above zero, the guest CHOSE a high',
  '  room category: acknowledge that the rate reflects the room selection',
  '  as well as the property. Never criticise the hotel for a room the',
  '  guest picked.',
                ...
                properties: {
                  ...
                  assessment: {
                    ...
                    properties: {
                      ...
                      evidence_used: {
                        type: 'array',
                        // Pinned in the SCHEMA for the same reason the
                        // hotel-value list is: validateAssessment rejects an
                        // unknown key, and a rejected draft costs the customer
                        // the reasoned verdict for a typo the schema could
                        // have prevented.
                        items: {
                          type: 'string',
                          enum: [
